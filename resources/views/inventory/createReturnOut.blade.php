...
<head>
    ...
    <title>Tambah Data Return Out</title>
</head>
<body>
    ...
    <div class="container"> 
        <div class="mt-4"> 
            <section class="content"> 
                ...
                @endif 
                <h4>Tambah Data Product New Return OUT</h4>
                <form action="{{ route('returnOut.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="type" id="type" value="OUT">
                    <input type="hidden" name="userid" id="userid" value="{{$user->userIDNo}}">
                    <div class="row g-3">
                        ...
                        <div class="form-group col-md-4">
                            <label for="perusahaan">Supplier Name</label>
                            <select class="form-control custom-select" name="productID" id="productID">
                                @foreach($supplier as $list)
                                    <option value="{{$list->supplier}}">{{$list->supplierName}} - {{$list->alamat}}</option>
                                @endforeach
                            </select>
                        </div>
